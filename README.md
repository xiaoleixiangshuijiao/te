...
- 🎯 Entry points to execute your program using `python -m <te>` or `$ te` with basic CLI argument parsing.
...
---
# te Flask Application

Awesome te created by xiaoleixiangshuijiao
...
```bash
git clone https://github.com/xiaoleixiangshuijiao/te te
cd te
make install
```
...
```bash
te create-db   # run once
te populate-db  # run once (optional)
te add-user -u admin -p 1234  # ads a user
te run
```
